...
server {
    ...
    listen 80;
    # listen 8080;
    server_name %PROJECT%.test %PROJECT%.com www.%PROJECT%.com;

    access_log  ${'nginx.log.directory'}/%PROJECT%.access.log;
    error_log  ${'nginx.log.directory'}/%PROJECT%.error.log;

    root ${'%PROJECT%.root.directory'}/%PROJECT%;
    ...
    location ~ ^/(www-status|ping)$ {
        ...
        include       ${'%PROJECT%.root.directory'}/data/conf/fastcgi.conf;
        fastcgi_pass   unix:${'phpfpm.socket'}/php-fpm-%PROJECT%.sock;
    }

    location  / {
        try_files \$uri /index.php =404;
        fastcgi_param  QUERY_STRING  \$query_string;
        fastcgi_pass   unix:${'phpfpm.socket.directory'}/php-fpm-%PROJECT%.sock;
        include       ${'%PROJECT%.root.directory'}/data/conf/fastcgi.conf;
    }
}
